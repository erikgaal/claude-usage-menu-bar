...
final class AccountStore: ObservableObject {
    ...
    private static let weeklyExhaustedPercent = 99.5
    ...
    var bestAccountIDs: Set<String> {
        ...
        where providerAccounts.count >= 2 {
            ...
            let candidates: [(id: String, sessionPercent: Double)] = providerAccounts
                .compactMap { account in
                    guard let state = states[account.id],
                        !state.needsReauth,
                        let session = Self.sessionLimit(in: state.limits)
                    else { return nil }
                    let weeklySpent = state.limits.contains {
                        $0.id != session.id && $0.percent >= Self.weeklyExhaustedPercent
                    }
            ...
            if candidates.count >= 2,
                candidates[1].sessionPercent - winner.sessionPercent < Self.bestAccountMargin {
    ...
    private static func sessionLimit(in limits: [LimitStatus]) -> LimitStatus? {
